...
class AirCargoProblem(Problem):
    ...
    def actions(self, state: str) -> list:
        ...
        # fluent state object from string state
        ...
        for action in self.actions_list:
            ...
            for clause in action.precond_pos:
                #if clause not in KB, it isn't possible
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        # The number of actions needs to satify all goal states will always be greater or equal to the
        # the number of goal expressions left to satisfy at a given state[1]
        # [1] https://discussions.udacity.com/t/understanding-ignore-precondition-heuristic/225906/12
        count = 0

        kb = PropKB()
        kb.tell(decode_state(node.state, self.state_map).pos_sentence())

        # count the number of unsatisfied goal states
        count = 0
        for clause in self.goal:
            if clause not in kb.clauses:
                count += 1
...
def air_cargo_p2() -> AirCargoProblem:
    ...
    :return: AirCargoProblem with initial state
    """
...
def air_cargo_p3() -> AirCargoProblem:
    ...
    :return: AirCargoProblem with initiaprl state
    """
    cargos = ['C1', 'C2', 'C3', 'C4']
    planes = ['P1', 'P2']
    airports = ['JFK', 'ATL', 'SFO', 'ORD']
